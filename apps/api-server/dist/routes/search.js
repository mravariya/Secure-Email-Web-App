...
export async function registerSearchRoutes(app) {
    ...
    app.get('/api/v1/search', { preHandler: [authenticate] }, async (req, reply) => {
        const userId = req.userId;
        const { q, folder, from, to, date_from, date_to, limit = '50', offset = '0', } = req.query;
        ...
        if (from)
            conditions.push(like(emails.fromAddress, `%${from}%`));
        if (to)
            conditions.push(like(emails.toAddresses, `%${to}%`));
        if (date_from) {
        ...
        if (q) {
            conditions.push(or(like(emails.fromAddress, `%${q}%`), like(emails.toAddresses, `%${q}%`), like(emails.subjectPlain, `%${q}%`), like(emails.subjectEncrypted, `%${q}%`)));
        }
