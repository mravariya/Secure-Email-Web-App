...
export async function registerEmailRoutes(app: FastifyInstance) {
  ...
  app.post<{
  ...
  }>('/api/v1/emails', { preHandler: [authenticate] }, async (req, reply) => {
    ...
    if (draftId) {
      const [existing] = await db
        ...
        .from(emails)
        .where(
          and(eq(emails.id, draftId), eq(emails.mailboxId, mailbox.id))
        )
        .limit(1);
  ...
  app.get<{
    Params: { id: string };
  }>('/api/v1/emails/:id', { preHandler: [authenticate] }, async (req, reply) => {
    const userId = req.userId!;
  ...
  app.patch<{
    Params: { id: string };
    Body: { isRead?: boolean; isStarred?: boolean; folder?: string };
  }>('/api/v1/emails/:id', { preHandler: [authenticate] }, async (req, reply) => {
    const userId = req.userId!;
    const emailId = req.params.id;
