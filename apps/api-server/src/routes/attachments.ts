...
export async function registerAttachmentRoutes(app: FastifyInstance) {
  ...
  app.post<{
    Querystring: { emailId?: string };
  }>('/api/v1/attachments/upload', { preHandler: [authenticate] }, async (req, reply) => {
    ...
    if (emailId) {
      const [email] = await db
        ...
        .limit(1);
      if (email) resolvedEmailId = email.id;
    }
    if (!resolvedEmailId) {
  ...
  app.get<{
    Params: { id: string };
  }>('/api/v1/attachments/:id', { preHandler: [authenticate] }, async (req, reply) => {
    const userId = req.userId!;
    ...
    try {
      ...
      reply.header(
        'Content-Disposition',
        `attachment; filename="${encodeURIComponent(attachment.filenameEncrypted || 'download')}"`
      );
