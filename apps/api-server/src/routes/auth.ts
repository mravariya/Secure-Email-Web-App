...
export async function registerAuthRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    const [user] = await db
      ...
      .returning();
    if (!user) return reply.status(500).send({ error: { code: 'INTERNAL', message: 'Insert failed' } });
  ...
  app.post<{
    Body: { email: string; password: string };
  }>('/api/v1/auth/login', async (req, reply) => {
    const { email, password } = req.body || {};
    if (!email || !password) {
      return reply.status(400).send({
  ...
  }>('/api/v1/auth/refresh', async (req, reply) => {
  ...
  app.post<{
    Body: { token: string };
  }>('/api/v1/auth/verify-email', async (req, reply) => {
  ...
  app.post<{
    Body: { email: string };
  }>('/api/v1/auth/forgot-password', async (req, reply) => {
  ...
  app.post<{
    Body: { token: string };
  }>('/api/v1/auth/2fa/verify', { preHandler: [authenticate] }, async (req, reply) => {
    const userId = req.userId!;
    const token = req.body?.token;
    if (!token) {
      return reply.status(400).send({
  ...
  }>('/api/v1/auth/2fa/disable', { preHandler: [authenticate] }, async (req, reply) => {
    const userId = req.userId!;
