...
export async function registerUserKeysRoutes(app: FastifyInstance) {
  ...
  }>('/api/v1/users/keys', { preHandler: [authenticate] }, async (req, reply) => {
  ...
  }>('/api/v1/users/public-key', { preHandler: [authenticate] }, async (req, reply) => {
